...
def create_upload(concat_df):
    file = csv.writer(open(file_path('upload.csv'), 'w', newline=''))
    file.writerow(('Employee Name', 'Employee Number', 'Department', '_diff'))
    for index, rows in concat_df.iterrows():
        rows = rows[rows != 0]
        split_employee = str.split(index, '(')
        employee_name = split_employee[0].strip()
        employee_id = split_employee[1].strip(')')
        ...
        for key,value in dept_default_dict.items():
            file.writerow((employee_name, int(employee_id), key, value))

def prettify_df(concat_df):
...
def fix_horrific_overtime_calc_mess(file_path_org, file_path_recalc, filter_columns_to_keep, filter_columns_to_drop):
    ...
    create_upload(concat_df)
    prettify_df(concat_df)
